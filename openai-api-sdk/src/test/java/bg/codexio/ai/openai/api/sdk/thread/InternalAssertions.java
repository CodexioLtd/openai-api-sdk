...
public class InternalAssertions {
    ...
    static final ThreadRequestBuilder<ThreadModificationRequest> THREAD_MODIFICATION_REQUEST_THREAD_REQUEST_BUILDER = ThreadRequestBuilder.<ThreadModificationRequest>builder()
                                                                                                                                          .withSpecificRequestCreator(threadRequestBuilder -> new ThreadModificationRequest(threadRequestBuilder.metadata()));


    static void createThreadRequestIsPopulated(ThreadConfigurationStage<ThreadCreationRequest> nextStage) {
        var request = nextStage.requestBuilder.specificRequestCreator()
                                              .apply(nextStage.requestBuilder);
    ...
    static void modifyThreadRequestIsPopulated(ThreadConfigurationStage<ThreadModificationRequest> nextStage) {
        var request = nextStage.requestBuilder.specificRequestCreator()
                                              .apply(nextStage.requestBuilder);
    ...
    static void execute(ThreadConfigurationStage<ThreadCreationRequest> threadConfigurationStage) {
        when(threadConfigurationStage.httpExecutor.execute(any())).thenAnswer(res -> THREAD_RESPONSE);
    }

    static void executeWithPathVariable(ThreadConfigurationStage<ThreadModificationRequest> threadConfigurationStage) {
        when(threadConfigurationStage.httpExecutor.executeWithPathVariable(
                any(),
                any()
